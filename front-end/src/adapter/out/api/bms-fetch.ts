import { BMS_API_URL } from "@/lib/api-base";
...
async function fetchOnce(path: string, init?: RequestInit): Promise<Response> {
  ...
  return fetch(`${BMS_API_URL}${path}`, { ...init, headers });
}

export async function bmsFetchJson(path: string, init?: RequestInit): Promise<unknown> {
  let res: Response;
  try {
