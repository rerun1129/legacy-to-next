import type { CreateNonBlRequest, UpdateNonBlRequest, NonBlFreightLineRequest } from '@/domain/non-bl';
...
/** FreightRow 배열 → NonBlFreightLineRequest 배열 변환 */
function buildFreightLines(rows: FreightRow[] | undefined): NonBlFreightLineRequest[] | undefined {
...
export function buildNonBlRequest(values: NonBlFormValues): CreateNonBlRequest {
  ...
  return {
    ...
    freightSelling: buildFreightLines(values.freightSelling as FreightRow[] | undefined),
    freightBuying:  buildFreightLines(values.freightBuying as FreightRow[] | undefined),
  };
}
