...
export function NonBLEntry() {
  ...
  const handleFreightMutated = useCallback(() => {
    ...
    if (entry.id != null) {
      queryClient.invalidateQueries({ queryKey: ["non-bl", "detail", entry.id] });
    }
  }, [entry, queryClient]);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: entry.tab === "freight" ? "contents" : "none" }}><FreightTab   key={`${entry.resetVersion}:${entry.nonce ?? 0}`} active={entry.tab === "freight"} layoutScope="non-bl-entry.freight" blType="HOUSE" blId={entry.id ?? null} onFreightMutated={handleFreightMutated} /></div>
    </form>
