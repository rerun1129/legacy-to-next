...
import {
  buildHsCodeColumns,
  getHsCodeRowClassName,
  type HsCodeFormRow,
  type FormValues,
} from "./hs-code-grid-columns";

const DEFAULT_FILTER: HsCodeFilter = {
  hsCode: "",
  name: "",
  scope: "ALL",
};
...
const PASTE_COLS = ["hsCode", "name", "nameEn", "countryCode", "active"] as const;

type HsCodeSearchState = SavedSearchState & { extraFilter: HsCodeFilter | null };

const ROW_IS_EQUAL = (a: HsCodeFormRow, b: HsCodeFormRow) =>
  a.name === b.name &&
  a.nameEn === b.nameEn &&
  a.countryCode === b.countryCode &&
  a.active === b.active;

const TO_CREATE = (row: HsCodeFormRow) => ({
  hsCode: row.hsCode,
  name: row.name,
...
const TO_UPDATE = (row: HsCodeFormRow) => ({
  id: row.entityId,
  name: row.name,
  nameEn: row.nameEn || null,
  countryCode: row.countryCode || null,
...
export function HsCodeListClient() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["admin-code-hs-code", "list", extraFilter, currentPage],
    queryFn: () => hsCodeUseCases.search(extraFilter!, currentPage, DEFAULT_PAGE_SIZE),
  ...
  const originalRows = useMemo<HsCodeFormRow[]>(
  ...
  function handleAdd() {
    const id = -Date.now();
    append({ entityId: id, hsCode: "", name: "", nameEn: "", countryCode: "", active: true });
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const td = document.querySelector(
        `td[data-row-key="${key}"][data-col-key="hsCode"]`,
      ) as HTMLElement | null;
  ...
  const invalidateList = () => qc.invalidateQueries({ queryKey: ["admin-code-hs-code", "list"] });
  ...
  const saveChangesMutation = useMutation({
    mutationFn: () => {
      ...
      return hsCodeUseCases.saveChanges({
    ...
    <>
      ...
      >
        ...
        <div className="list-wrap">
          <GridList<HsCodeFormRow>
            columns={columns}
            data={fields as unknown as HsCodeFormRow[]}
            rowKey={(row) => row.entityId}
            rowClassName={(row) => getHsCodeRowClassName(row, originalRows)}
