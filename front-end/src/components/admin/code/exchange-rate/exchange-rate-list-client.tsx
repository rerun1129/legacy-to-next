...
import {
  ...
  getExchangeRateRowClassName,
  type ExchangeRateFormRow,
  type FormValues,
} from "./exchange-rate-grid-columns";
import {
  PASTE_COLS,
  ROW_IS_EQUAL,
  TO_CREATE,
  TO_UPDATE,
  toFormRow,
} from "./exchange-rate-list-helpers";

const DEFAULT_FILTER: ExchangeRateFilter = {
  fromCurrencyCode: "",
  toCurrencyCode: "",
  name: "",
  scope: "ALL",
...
export function ExchangeRateListClient() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["admin-code-exchange-rate", "list", extraFilter, currentPage],
    queryFn: () => exchangeRateUseCases.search(extraFilter!, currentPage, DEFAULT_PAGE_SIZE),
  ...
  const originalRows = useMemo<ExchangeRateFormRow[]>(
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const td = document.querySelector(
        `td[data-row-key="${key}"][data-col-key="fromCurrencyCode"]`,
      ) as HTMLElement | null;
  ...
  const invalidateList = () => qc.invalidateQueries({ queryKey: ["admin-code-exchange-rate", "list"] });
  ...
  const saveChangesMutation = useMutation({
    mutationFn: () => {
      ...
      return exchangeRateUseCases.saveChanges({
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 8 }}>
        ...
        />
        <ActionButton
          buttonCode="BTN_ADMIN_CODE_EXCHANGE_RATE_SAVE"
      ...
      <ExchangeRateListFilter form={filterForm} />
      ...
      >
        ...
        <div className="list-wrap">
          <GridList<ExchangeRateFormRow>
            columns={columns}
            data={fields as unknown as ExchangeRateFormRow[]}
            rowKey={(row) => row.entityId}
            rowClassName={(row) => getExchangeRateRowClassName(row, originalRows)}
