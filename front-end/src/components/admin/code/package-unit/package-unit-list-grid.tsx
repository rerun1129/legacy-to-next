...
import type { PackageUnitRow, PackageUnitFilter } from "@/domain/code/package-unit";

interface Props {
  extraFilter: PackageUnitFilter | null;
  currentPage: number;
...
export function PackageUnitListGrid({ extraFilter, currentPage, onPageChange, onRowDoubleClick, selectedKeys, onSelectionChange }: Props) {
  ...
  const { data, isFetching, error } = useQuery({
    queryKey: ["admin-code-package", "list", extraFilter, currentPage],
    queryFn: () => packageUnitUseCases.search(extraFilter!, currentPage, 50),
    ...
    return (
      <div className="panel" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
        <div className="panel__head">
          ...
          <span className="panel__title">포장단위 관리</span>
        </div>
        <div
          className="list-wrap"
  ...
  return (
    ...
    >
      <div className="panel__head">
        ...
        <ColumnVisibilityMenu<PackageUnitRow> gridId="admin-code-package" defaultColumns={COLUMNS} />
      </div>
      <div className="list-wrap">
        <GridList<PackageUnitRow>
          columns={COLUMNS}
          data={rows}
