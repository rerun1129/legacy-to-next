...
export function PackageUnitListFilter({ form }: Props) {
  const t = useTranslations("admin.packageUnit.filter");
  ...
  const { data: suggestions = [] } = useQuery({
    queryKey: ["admin-code-package-unit", "autocomplete", acQuery],
    queryFn: () => packageUnitUseCases.autocomplete(acQuery),
    enabled: acQuery.length >= 1,
    staleTime: 30_000,
  });

  function handleSelect(item: CodeBoxSuggestion) {
    setValue("packageCode", item.code);
  }
      ...
      <div className="search-card__body">
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
          <CodeBox
            ...
            codeProps={{ placeholder: t("packageCodePlaceholder"), ...register("packageCode") }}
          />
          <div className="lcn">
