...
import { PortListGrid } from "./port-list-grid";
import { PortEntryModal } from "./port-entry-modal";
import type { EntryModalState } from "./port-entry-modal";
import type { PortFilter } from "@/domain/code/port";
import { portUseCases } from "@/application/code/port/use-cases";
import { confirm } from "@/components/confirm";
import { toast } from "@/lib/toast-store";

const DEFAULT_VALUES: PortFilter = {
  portCode: "",
  name: "",
  countryCode: "",
  portType: "ALL",
...
export function PortListClient() {
  const form = useForm<PortFilter>({ defaultValues: DEFAULT_VALUES });
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: (ids: number[]) => portUseCases.deleteMany(ids),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-code-port", "list"] });
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 8 }}>
        ...
        />
        <ActionButton
          buttonCode="BTN_ADMIN_CODE_PORT_CREATE"
          className="btn btn--modal btn--sm"
