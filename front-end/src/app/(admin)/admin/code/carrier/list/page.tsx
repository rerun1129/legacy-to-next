"use client";

import { Ship } from "lucide-react";
import { AdminGuard } from "@/components/admin/admin-guard";
import { CarrierListClient } from "@/components/admin/code/carrier/carrier-list-client";

export default function AdminCarrierListPage() {
  return (
    <AdminGuard requiredMenuCode="MENU_ADMIN_CODE_CARRIER">
      <div className="app__main--list" style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, overflow: "hidden" }}>
        <div className="page-head">
          <div className="page-head__title">
            <div className="page-head__title-icon"><Ship size={14} /></div>
            Carrier Management
          </div>
        </div>
        <div style={{ padding: 16, display: "flex", flexDirection: "column", minHeight: 0, flex: 1 }}>
          <CarrierListClient />
        </div>
      </div>
    </AdminGuard>
  );
}
