...
타입 시스템과 코드 흐름상 절대 null 일 수 없는 곳에 `?.`나 `if (x !== null)` 이 있으면 위반.
...
**적용 범위**: 백엔드·프론트엔드 도메인 코드(`domain/`, `application/`, `adapter/`)에만 적용.

### 의존 방향 (필수)

- **Adapter → Application → Domain** 만 허용
- **역방향 절대 금지** (Domain 이 Application 을 import → 위반)
- **Domain 은 외부 의존 0**: 프레임워크, ORM, HTTP 클라이언트, 파일 I/O, 환경변수, 시간(`Date.now()`, `new Date()`) 직접 사용 금지
- **Application 은 Port 인터페이스만 의존**, 어댑터 구현체를 직접 import 금지

## ARCH2. 파일 크기
500줄 초과 절대 금지.
예외:
- 자동 생성 파일 (마이그레이션, OpenAPI 클라이언트, GraphQL 코드젠 등)
...
Adapter(in)에서 외부 형식 → 도메인 변환, Adapter(out)에서 도메인 → 외부 형식 변환.

## ARCH5. 시간/난수/UUID 의 추상화

`Date.now()`, `Math.random()`, `crypto.randomUUID()` 직접 호출 금지. Clock/IdGenerator 포트로 추상화.
...
- HTTP body, query, URL param, form, file, env, 외부 API 응답은 사용 전 검증
- 검증 실패 시 도메인 에러로 변환하여 throw
- 검증은 어댑터 진입 직후. Use Case 안에서 다시 검증할 필요 없음
- 가능하면 zod, yup, class-validator, Pydantic 등 스키마 라이브러리 사용

## C4. 자원 해제

파일 핸들, DB 커넥션/트랜잭션, 네트워크 소켓, 타이머는 try/finally
또는 context manager 로 해제.

## C5. SQL 정확성

- N+1 쿼리 패턴 금지. ORM 사용 시 명시적 `include`/`join`/`prefetch`.
...
- 공유 가변 상태에 대한 동시 접근 명시적 처리 (락, 큐, 메시지 패싱)
- DB 트랜잭션 격리 수준이 의도와 맞는지 확인 (SELECT FOR UPDATE 누락, read-committed 가정 위반 등)
- 외부 시스템 호출은 멱등성(idempotency) 고려. 재시도 시 중복 실행 안전한지.

## C8. 로그 위생

- 비밀번호, 토큰, API 키, PII 출력 금지
- 운영 로그에 `console.log` 디버그 흔적 커밋 금지
- 에러 로그는 stack trace + 식별자 + 컨텍스트 포함
- 구조화 로그 사용 권장 (JSON 로그). string concat 보다 메타데이터 필드.
