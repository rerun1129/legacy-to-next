...
CREATE TABLE IF NOT EXISTS master_bl_air (
    ...
    volume_divisor            VARCHAR(10)  DEFAULT 'CM/6000'
);
...
COMMENT ON COLUMN master_bl_dim.seq               IS '그리드 행 순번';
...
COMMENT ON COLUMN master_bl_desc.marks_left        IS 'Marks and Numbers 좌측';
COMMENT ON COLUMN master_bl_desc.marks_right       IS 'Marks and Numbers 우측';
COMMENT ON COLUMN master_bl_desc.description_left  IS '화물 설명 좌측 (해상=Description, 항공=Nature & Quantity of Goods)';
COMMENT ON COLUMN master_bl_desc.description_right IS '화물 설명 우측';
...
COMMENT ON COLUMN master_bl_schedule_leg.seq           IS '그리드 표시 순서 — 첫 행=ETD, 마지막 행=POD/ETA 파생 기준';

CREATE INDEX IF NOT EXISTS idx_master_bl_schedule_leg_master_bl_id ON master_bl_schedule_leg(master_bl_id);

-- =============================================================================
-- E-12 House B/L 치수 (항공 전용, 1:N)
-- =============================================================================
...
COMMENT ON COLUMN house_bl_dim.seq               IS '그리드 행 순번';
...
COMMENT ON COLUMN house_bl_desc.marks_left        IS 'Marks and Numbers 좌측';
COMMENT ON COLUMN house_bl_desc.marks_right       IS 'Marks and Numbers 우측';
COMMENT ON COLUMN house_bl_desc.description_left  IS '화물 설명 좌측';
COMMENT ON COLUMN house_bl_desc.description_right IS '화물 설명 우측';
...
COMMENT ON COLUMN house_bl_license.seq                    IS '그리드 행 순번';

CREATE INDEX IF NOT EXISTS idx_house_bl_license_house_bl_id ON house_bl_license(house_bl_id);

-- =============================================================================
-- E-18 House B/L 참고번호 (1:N)
-- =============================================================================
CREATE TABLE IF NOT EXISTS house_bl_reference (
    house_bl_reference_id  BIGINT        GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    house_bl_id            BIGINT        NOT NULL REFERENCES house_bl(house_bl_id),
...
COMMENT ON COLUMN house_bl_reference.house_bl_id      IS 'House B/L 참조 FK';
COMMENT ON COLUMN house_bl_reference.reference_type   IS '참조 유형 (화주관리번호, PO, PRO 등)';
COMMENT ON COLUMN house_bl_reference.reference_no     IS '참조 번호 값';
COMMENT ON COLUMN house_bl_reference.seq              IS '그리드 행 순번';

CREATE INDEX IF NOT EXISTS idx_house_bl_reference_house_bl_id ON house_bl_reference(house_bl_id);
...
COMMENT ON COLUMN house_bl_schedule_leg.seq          IS '그리드 표시 순서 — 첫 행=ETD, 마지막 행=POD/ETA 파생 기준';
...
-- E-16  HS Code
-- E-23  거래처 마스터 (Party Master) — MDM 모듈 이관 예정
-- =============================================================================
