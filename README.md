...
**Phase 1 (FMS) 완료 기준** — 3개 서브도메인(Master B/L · House B/L · Switch B/L) · 24개 도메인 엔티티 · 20개 테이블 · 약 453개 백엔드 테스트 케이스 · Vitest 65 + Playwright E2E 4 spec · 헥사고널 4계층 ArchUnit 강제 · OpenAPI 3.x · RFC 7807.
...
OpenAPI 3.x · RFC 7807 problem+json · Zod 4 런타임 응답 검증 · JSON 구조화 로깅 (`trace_id`/`service`)
...
| **Planned** | Spring Security + JWT · Spring Batch 5 · AWS 인프라 · MongoDB Analytics Mart (PFM) |
...
| FMS API (Swagger) | http://localhost:8080/swagger-ui.html |
| Admin API | http://localhost:8081 |
| PostgreSQL | localhost:5432 (fms / fms_local) |

```bash
docker compose down          # 중지 (DB 데이터 유지)
docker compose down -v       # 중지 + DB 초기화
docker compose up --build fms-api   # 특정 서비스만 재빌드
```
...
├── schema/                   docker-init 스크립트 · 과거 수동 마이그레이션 이력 (FMS DDL 정본: back-end/java-spring/src/main/resources/db/migration)
