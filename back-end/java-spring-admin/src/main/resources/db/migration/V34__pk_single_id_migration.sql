...
-- =============================================================================
-- 2. admin.attribute_value 단일 PK 전환
-- =============================================================================

-- 2-1. 단일 id 컬럼 추가 (GENERATED ALWAYS AS IDENTITY, V11 attribute_definition 과 동일 컨벤션)
...
-- 2-4. (attribute_key, value) 보조 UNIQUE 제약 — 의미(무결성) 유지
ALTER TABLE admin.attribute_value
    ADD CONSTRAINT uq_admin_attribute_value_key_value UNIQUE (attribute_key, value);

-- fk_admin_attribute_value_key 는 기존 그대로 유지 (attribute_key → attribute_definition.attribute_key)

-- =============================================================================
-- 3. permission_preset_attribute_value FK 재매핑
-- =============================================================================

-- 3-1. attribute_value_id 컬럼 추가 (NULL 허용으로 시작 — 후속 UPDATE 로 채움)
ALTER TABLE admin.permission_preset_attribute_value
    ADD COLUMN attribute_value_id BIGINT;

-- 3-2. 기존 (attribute_key, av_value) → attribute_value.id 매핑
...
BEGIN
    ...
    -- 4-2. admin_user_permission_backup 는 V16 에서 테이블이 DROP 되어 0건이 정상
    ...
    -- 4-3. permission_preset_attribute_value.attribute_value_id NULL 행 0 확인
