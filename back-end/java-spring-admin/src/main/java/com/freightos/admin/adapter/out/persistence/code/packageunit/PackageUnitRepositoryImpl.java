...
public class PackageUnitRepositoryImpl implements PackageUnitRepositoryCustom {
    ...
    public PagedResult<PackageUnitSummary> searchSummaries(SearchPackageUnitCommand command) {
        ...
        Root<PackageUnitJpaEntity> countRoot = countQuery.from(PackageUnitJpaEntity.class);
        ...
        CriteriaQuery<PackageUnitJpaEntity> dataQuery = cb.createQuery(PackageUnitJpaEntity.class);
        Root<PackageUnitJpaEntity> dataRoot = dataQuery.from(PackageUnitJpaEntity.class);
        dataQuery.where(buildPredicates(cb, dataRoot, command));
        // package_code asc + id asc tie-break — T1 flaky 방지
        dataQuery.orderBy(cb.asc(dataRoot.get("packageCode")), cb.asc(dataRoot.get("id")));

        TypedQuery<PackageUnitJpaEntity> typedQuery = em.createQuery(dataQuery);
    ...
    private Predicate[] buildPredicates(CriteriaBuilder cb, Root<PackageUnitJpaEntity> root, SearchPackageUnitCommand command) {
        ...
        if (StringUtils.hasText(command.packageCode())) {
            predicates.add(cb.like(root.get("packageCode"), command.packageCode() + "%"));
        }
