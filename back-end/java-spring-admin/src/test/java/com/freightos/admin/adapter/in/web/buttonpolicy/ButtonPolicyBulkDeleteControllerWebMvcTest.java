...
@WebMvcTest(controllers = ButtonPolicyController.class)
@Import({ SecurityConfig.class, JwtAuthenticationFilter.class })
class ButtonPolicyBulkDeleteControllerWebMvcTest {
    ...
    @MockitoBean
    private ButtonPolicyUseCase buttonPolicyUseCase;

    @MockitoBean
    private ButtonPolicyAssembler buttonPolicyAssembler;
    ...
    void bulkDelete_unauthenticated_returns401() throws Exception {
        ...
        mockMvc.perform(delete("/api/admin/access/button-policy/bulk")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isUnauthorized());
    }

    // ── BTN_ADMIN_ACCESS_POLICY_DELETE authority → 200 ───────────────────────

    @Test
    @WithMockUser(authorities = {"ROLE_ADMIN", "BTN_ADMIN_ACCESS_POLICY_DELETE"})
    void bulkDelete_validRequest_returns200() throws Exception {
        willDoNothing().given(buttonPolicyUseCase).deleteButtonPoliciesByIds(any());
