...
@WebMvcTest(controllers = CodeMasterController.class)
@Import({ SecurityConfig.class, HeaderAuthenticationFilter.class })
class CodeMasterBulkDeleteControllerWebMvcTest {
    ...
    void bulkDelete_unauthenticated_returns401() throws Exception {
        ...
        mockMvc.perform(delete("/api/admin/code-master/bulk")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isUnauthorized());
    }

    // ── BTN_ADMIN_CODE_LIST_SAVE authority → 200 ────────────────────────────────

    @Test
    @WithMockUser(authorities = "BTN_ADMIN_CODE_LIST_SAVE")
    void bulkDelete_validRequest_returns200() throws Exception {
        willDoNothing().given(codeMasterUseCase).deleteCodeMasters(any());
