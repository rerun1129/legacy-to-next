...
public interface FreightInputPort {
     ...
     */
    void saveFreight(FreightBlType blType, String blId, FreightInputCommand cmd);

    /** blType+blId로 운임 헤더+라인 조회. 헤더 없으면 Optional.empty(). */
    Optional<FreightView> findFreightByBl(FreightBlType blType, String blId);

    /** 해당 B/L에 운임 라인이 1건 이상 존재하는지 확인. */
    boolean existsFreightLines(FreightBlType blType, String blId);

    /** 해당 B/L의 헤더+라인 전체 삭제. */
    void deleteFreight(FreightBlType blType, String blId);
}
