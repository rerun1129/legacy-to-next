 ...
 * 차이가 있으면 WARN 로그를 남기고 앱은 계속 기동한다.
 * DB 조회 불가 시 체크를 스킵하고 WARN만 남긴다.
 */
@Slf4j
@Component
public class EnumDriftChecker {
    ...
    public void checkDrift() {
        Set<String> registryKeys = enumRegistry.getAllKeys();
        for (String groupCode : registryKeys) {
    ...
    private void checkGroup(String groupCode) {
        ...
        if (dbOptions.isEmpty()) {
            // DB에 그룹 자체가 없는 것은 admin 아직 시드 전일 수 있음 — 조용히 넘김
            return;
        }
