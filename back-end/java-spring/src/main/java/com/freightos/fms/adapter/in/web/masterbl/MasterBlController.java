...
public class MasterBlController {
    ...
    public ResponseEntity<ApiResponse<MasterBlDetailResponse>> updateMasterBl(
            ...
            @Valid @RequestBody UpdateMasterBlRequest request) {
        MasterBl entity = masterBlUseCase.findMasterBlById(id);
        MasterBl saved = masterBlUseCase.save(entity);
        return ResponseEntity.ok(ApiResponse.of(masterBlAssembler.toDetail(masterBlUseCase.findMasterBlDetailById(saved.getId()))));
    }
