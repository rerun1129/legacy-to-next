...
다음 11가지를 **plan 단계에서 명시적으로 작업 범위에 포함**하라. 누락 시 사용자 검수 단계에서 회귀가 드러나 재작업한다.
...
8. **BE detail response ↔ form.reset nested 매핑 정합 (Read flow 결정)** — 신규/변경 form 필드 추가 직후 (a) BE response DTO record에 필드 존재 grep (b) `form.reset({...})` 객체의 nested block(예: `seaDetail: { ... }`) 존재 read (c) zod schema 키 ⊆ form.reset 매핑 키 정합. 누락 시 BE/FE 풀스택 작업 단위 추가.
...
- **Sea House Entry Change B/L No 캐시 invalidate 중복 + toolbar HBL No 미갱신 fix (2026-05-14)** — `house-bl-entry.tsx`의 `HouseChangeBlNoModal` `onChanged` 콜백에서 `queryClient.invalidateQueries(["house-bl","detail",id])` 호출 제거. 모달 자체(`house-change-bl-no-modal.tsx:40`)가 같은 queryKey에 대해 invalidate를 이미 수행 중이라 부모 콜백의 중복 호출이 fetch 2회 발사 → BE detail SELECT 4건 × 2 = **8건 발생**. 동시에 1차 fetch가 `detailLoadedRef.current = true`(직전 detail 로드 후 §6.49 ⑨ 가드로 설정된 상태)에서 시작되어, useEffect의 form.reset 가드(`if (detailLoadedRef.current) return;`)에 막혀 새 hblNo로 form.reset이 **스킵**되는 race가 발생, toolbar HBL No 필드가 갱신되지 않음(사용자 보고로 확정). 부모(Entry)는 `detailLoadedRef.current = false`로 form.reset 재발동만 책임 — 동기 흐름: 모달 mutation onSuccess → `invalidate`(1회) → `onChanged?.()` 동기 호출 → 부모에서 `detailLoadedRef.current = false` 설정 → fetch 완료 시점에는 ref가 항상 `false` → `form.reset(mapHouseBlDetailToForm(detail))` 안정 실행 → toolbar 갱신. 모달 측 invalidate는 단일 책임으로 유지(BE 응답 변경 시 캐시 일관성 책임은 mutation 호출자). SELECT 8 → 4로 절반 절감. 잔여 동일 패턴 점검 권장 — 다른 Entry의 `*ChangeBlNoModal onChanged` 콜백에서 부모 측 `invalidateQueries` 중복 호출이 있으면 동일 fix 적용(`change-bl-no-modal.tsx`/`truck-change-bl-no-modal.tsx`/Master 계열).

- **House B/L Entry toolbar Master Ref 바인딩 누락 풀스택 정합 (2026-05-14)** — Sea House Entry toolbar의 "Master Ref" 필드(form key `masterRefNo`)가 항상 빈 값으로 표시되던 결함. 저장 경로(Create/Update Command·Request·Assembler·`HouseBl.assignMasterReference`·`HouseBlDomainToJpaMapper.setMasterRefNo`·`HouseBlJpaToDomainMapper.assignMasterReference` + DB `master_ref_no` 컬럼)는 전부 정상 동작 중이었으나 **detail 조회 응답 경로 5축이 모두 비어 있었음** — §6.49 ⑧ 패턴의 정확한 사례. 풀스택 5파일 동시 정합: (BE) `HouseBlDetailResult` record 필드 + `HouseBlFactory.toDetailResult`의 `entity.getMasterRefNo()` 매핑 + `HouseBlDetailResponse` record 필드 + `HouseBlDetailResponse.from()` factory 인자. (FE) `domain/house-bl/index.ts`의 `HouseBlDetail` interface에 `masterRefNo?: string` 추가 + `adapter/out/api/house-bl.ts`의 `HOUSE_BL_DETAIL_SCHEMA`에 zod 파싱 라인 추가(이게 누락되어 BE가 보내도 FE 도메인 객체에서 무시되던 결정적 원인) + `map-house-bl-detail.ts:12`의 `detail.masterRefNo → form.masterRefNo` 매핑. 사전 점검 학습: **FE 도메인 타입과 zod adapter schema는 1:1 정합**이 SSOT — 도메인 타입에만 추가하고 adapter schema에 빼먹으면 BE 응답이 zod 파싱 시점에 소실. §6.49 ⑧의 점검 항목에 "FE adapter zod schema(`adapter/out/api/<domain>.ts`의 `*_DETAIL_SCHEMA`)에도 신규 필드 존재 여부 grep" 의무 추가.

- **ServiceTerm enum 매칭 비대칭 — Sea/Master fromLabel → valueOf 통일 (2026-05-14)** — Sea House Entry/Master B/L Entry에서 Service Term을 포함해 Save 시 BE에서 `IllegalArgumentException: Unknown service term code: CFS_CFS` 발생하던 결함(사용자 8080 로그 보고로 표면화). 원인: `EnumRegistryFactory:39-40` SSOT는 ServiceTerm을 `value=e.name()`("CFS_CFS"), `label=e.getLabel()`("CFS/CFS") 으로 노출 → FE form은 `e.name()` 값을 저장하고 그대로 update Command 전송. 그러나 BE 호출처 일부(`HouseBlSeaSubFactory:74`, `MasterBlFactory:203`)가 **`ServiceTerm::fromLabel`** 사용 — fromLabel은 label 매칭만 지원하므로 name 입력 거부. Truck 측(`HouseBlTruckSubFactory:32, 45`)은 이미 `ServiceTerm::valueOf` 사용 중이라 정상. fix: Sea/Master 호출처 2곳을 `ServiceTerm::valueOf`로 통일(Truck과 SSOT 정합). `ServiceTerm.fromLabel` 메서드 자체는 외부 호환 목적으로 enum 본체에 보존(현재 호출처 0건). `HouseBlSeaSubFactoryTest.java:138` 테스트 헬퍼의 입력값도 `"CY/CY"`(label) → `"CY_CY"`(name)으로 production 흐름과 정합(사용자 명시 승인). 점검 학습: **enum 매칭 함수 일관성** — 같은 enum에 대해 fromLabel/valueOf/fromCode가 호출처별로 비대칭일 경우 production 입력 형식과 어긋난 호출처가 throw. 신규 enum 도입 또는 enum 사용 호출처 추가 시 EnumRegistryFactory의 value 정의(`e.name()` vs `e.getCode()`)와 호출처 변환 함수(`valueOf` vs `fromCode` vs `fromLabel`)가 1:1로 정합해야 함. `feedback_enum_db_value` 메모리 + 본 절 SSOT.

- **House B/L update 응답 SEA void 파싱 완화 — @JsonInclude(NON_NULL) 정합 (2026-05-14)** — Sea House Entry에서 Save 시 화면에 `Invalid update response: [{"expected":"object","code":"invalid_type","path":["data"],"message":"Invalid input: expected object, received undefined"}]` 빨간 박스가 떴던 결함(직전 ServiceTerm fix로 update 자체가 BE 단에서 성공하게 되자 응답 파싱 결함이 표면화). 원인: BE `ApiResponse` 클래스에 `@JsonInclude(JsonInclude.Include.NON_NULL)`이 걸려 있어 SEA jobDiv update가 반환하는 `ApiResponse.ok(message)` (= `data=null, message=...`) 직렬화 시 **`data` 키 자체가 응답 JSON에서 제거됨** → 실제 응답은 `{"message": "..."}`. FE adapter(`house-bl.ts:184`)는 SEA void를 `z.object({ data: z.null() })`로 검사하지만 zod 입장에서 `data: undefined`는 `z.null()` 거부 → maybeVoid 실패 → 다음 줄 detail 파싱으로 폴백 → `expected object, received undefined` throw. fix: maybeVoid 검사를 `(json ?? {}).data == null`로 완화 (null/undefined 모두 void 처리). BE의 `@JsonInclude(NON_NULL)`은 다른 응답들에 폭넓게 걸려 있어 건드리지 않음(영향 범위 차단). 잔여 점검 권장 — `non-bl.ts`/`truck-bl.ts`/`master-bl.ts` 등 다른 adapter의 update/save 메서드에도 동일하게 `z.null()` 검사로 void 판정하는 패턴이 있다면 같은 fix 필요. 점검 학습: **BE `@JsonInclude(NON_NULL)` 정책 하에서는 null 필드가 응답에서 사라짐을 전제로 FE adapter 작성** — zod `z.null()`은 null만 매칭하므로 undefined 호환을 위해 `data == null` 패턴 또는 `z.null().optional()` 사용.

- **Sea House Entry Create 필수 검증 EXP/IMP 분기 — Bean Validation Groups 도입 (2026-05-14)** — Sea House Entry 신규 저장 시 BE에서 필수값 검증이 hblNo/polCode/podCode/etd/eta/actualCustomerCode/docPartnerCode/operatorCode/teamCode 9개 + shipmentType/bound 2개 = 총 11개 필드에 누락되어 빈 값/null도 통과하던 결함. UI required 시각 표시도 사용자 명시 목록(공통 10 + IMP Consignee 1)과 불일치했던 잉여 표시 6건(Vessel/Voyage/Liner/Freight Term/Incoterms 등) 정리 동반. 메모리 `feedback_validation_be_ssot` 정책(검증 책임 BE 일원화) + `feedback_enum_db_value` 정합. **BE 5파일**: (1) `adapter/in/web/validation/SeaGroup.java`/(2) `SeaImpGroup.java` 신규 — Bean Validation Group 인터페이스 2개, `SeaImpGroup extends SeaGroup`으로 IMP 검증 시 공통 그룹 함께 검증. (3) `CreateHouseBlRequest.java` — 공통 10개에 `@NotBlank(groups=SeaGroup.class)`, consigneeCode에 `@NotBlank(groups=SeaImpGroup.class)` 부착(기존 `@NotNull`/`@Size`/`@Pattern` 보존). (4) `HouseBlController.createHouseBl` — `jakarta.validation.Validator` 빈 주입 + `jobDiv=SEA` 진입 시 `bound=IMP`이면 SeaImpGroup, 그 외(EXP) SeaGroup 동적 그룹 검증, 위반 시 `ConstraintViolationException` throw. (5) `GlobalExceptionHandler.handleConstraintViolation` 신설 — 기존 `MethodArgumentNotValidException` 핸들러와 동일 ProblemDetail 응답 패턴(`[field] message`, `type=TYPE_BASE+VALIDATION_FAILED`, status=400, content-type=application/problem+json). **BE 테스트 1파일**: `HouseBlControllerWebMvcTest.createHouseBl_happyPath` 요청 본문에 SEA+EXP 필수 9개 더미 값 보강(사용자 명시 승인). **FE 3파일**: `schedule-panel.tsx` Vessel/Voyage/Liner 시각 required 제거(ETD/ETA/POL/POD는 유지), `sea-trade-panel.tsx`·`trade-panel.tsx` Incoterms/Freight Term 시각 required 제거. Consignee 동적 IMP-required는 `party-panel.tsx` + `widget-grid.tsx`의 `isExp` prop chain으로 이미 구현되어 있어 추가 변경 불필요. **메모리 정책 준수** — FE에 zodResolver/HTML5 required 추가 안 함(시각 클래스만 정합). 잔여 점검 권장 — Air/Truck House Entry, Sea Master Entry, Air Master Entry에 동일 패턴 적용은 별도 작업(`feedback_validation_be_ssot` 일관성).
...
- **Round 3 후속 결함 (open)**:
  - 자식 컬렉션(containers/scheduleLegs/licenses/airCharges/dims/truckOrders) BE detail 응답 노출 누락 — 그리드 패널 빈 상태. Sea Container 그리드부터 확인 필요.
  - AIR 본체 issue 필드(`HouseBlAir.issuePlace` 등) nested 응답 노출 — Phase A-1과 동일 패턴으로 추가.
  - `house-bl-entry.tsx` 464줄 (300 초과, 500 미만 — 강제 분리 아님) → 권장 분리 후보: page-head buttons / form.reset 매핑 / mutation 핸들러.
