...
사용자 요청
  ...
  → [Stop 훅] → .review_pending 확인 후 Reviewer 실행 (Sonnet 1차·ESCALATE 시 Opus 2차)
      · REJECTED → exit 2로 메인 재개 → 메인이 [Coder × N] 재호출 (`isolation: "worktree"`, base: 현재 트렁크 HEAD)
  ...
  → [QA]                         통합 빌드·테스트 (단일)
  → [메인] 사용자 결과 보고 후 종료
      → Stop 훅 재실행 (stop_hook_active=true, 마커 있음) → 마커 삭제 후 exit 0
```
...
| 사용자 → Coder × N | Agent 도구 병렬 호출 (`isolation: "worktree"`) | 단위별 기획 |
...
| Stop 훅 → Main (REJECTED) | exit 2 + stderr 피드백 주입 | 규칙 위반 목록 + 개선 제안 |
...
| 시점 | 명령어 |
|------|--------|
| 모든 Coder worktree 머지 후 (충돌 없음) | `git merge <branch>` → `touch .claude/.review_pending` |
