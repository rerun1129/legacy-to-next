...
2. **분기 결정용 jobDiv 추가 lookup** — `findEntityById`를 `findJobDivById`로만 줄여도 `SELECT job_div FROM master_bl WHERE master_bl_id=?` 1 query는 남는다. FE는 variant(예: `sea-exp`/`air-imp`)로 jobDiv를 이미 결정한 상태로 PUT body에 송신하므로 BE에서 다시 lookup할 이유가 없다.
3. **마지막 `findEntityById` reload** — Update 어댑터가 `void` 반환이라 응답 빌드 시 detail을 재구성하려고 entity를 다시 fetch. `master_bl`은 1차 캐시 hit으로 미발사이지만 `master_bl_sea` + `master_bl_sea_desc`는 query method 사용이라 SQL이 다시 발사 (§6.34 — 1차 캐시는 PK 기반 조회만).

**해결 — 두 곳 동시 fix 의무**

**1. jobDiv 분기는 `cmd.jobDiv()` 신뢰 — DB lookup 0건**

```java
// 안티패턴 1 — 전체 entity fetch 후 instanceof 분기
MasterBl entity = findEntityById(id);
switch (entity) { case MasterBlSea ignored -> ...; case MasterBlAir ignored -> ...; }

// 안티패턴 2 — jobDiv만 줄였지만 여전히 DB lookup 1 query
MasterBlJobDiv jobDiv = masterBlPort.findJobDivById(id).orElseThrow(...);

// 권장 — PUT body 의 cmd.jobDiv() 신뢰 (lookup 0 query)
...
**왜 cmd.jobDiv() 신뢰가 안전한가**:
- FE 는 variant 로 jobDiv 결정해 PUT body 에 항상 송신 (`buildCreateMasterBlPayload`의 `jobDiv: variant.mode`).
- 변조 시 fail-safe 두 곳:
  - **SEA 분기**: `SeaMasterUpdatePersistenceAdapter`가 내부에서 `parentJpa.getJobDiv() != MasterBlJobDiv.SEA` 검증 → `IllegalStateException`
  - **AIR 분기**: `saveMasterBl` 내부 `switch (domain)`의 `default -> throw IllegalArgumentException` — 도메인 타입은 DB 기반(`toAirDomain`/`toSeaDomain`)이라 cmd 변조에 영향 안 받음
- `MasterBlJobDiv.valueOf` 가 null/invalid 입력에 `IllegalArgumentException` 토출 — 추가 가드 불필요.
...
| 단계 | Before | After (jobDiv-lookup) | After (cmd.jobDiv 신뢰) |
|---|---|---|---|
| jobDiv 분기 | 3 (master_bl + sea + desc) | 1 (findJobDivById) | **0 (cmd.jobDiv valueOf)** |
| 어댑터 fetch | 2 (sea + desc) | 2 (동일) | 2 (동일) |
| 응답 reload | 2 (sea + desc) | 0 (domain 재사용) | 0 (동일) |
| consoled house 조회 | 2 (hbl summary + container) | 2 (동일) | 2 (동일) |
| **PUT 총** | **9** | **5** | **4** |
| GET refetch (FE invalidate) | 5 | 5 | 5 |
| **전체** | **14** | **10** | **9** |
...
grep -n "findEntityById\|findMasterBlById\|findHouseBlById" back-end/java-spring/src/main/java/com/freightos/fms/application/*/
...
# jobDiv 분기에 cmd.jobDiv() 가 아닌 DB lookup 사용하는지 확인
grep -n "findJobDivById" back-end/java-spring/src/main/java/com/freightos/fms/application/
```

**잔여 점검 권장**

Air Master / 차기 Entry 마이그레이션 시:
1. Service.updateXxx의 jobDiv 분기는 `MasterBlJobDiv.valueOf(command.jobDiv())` 로 cmd 신뢰 — DB lookup 금지
...
4. 어댑터 자체 jobDiv 검증 의무 (`if (parentJpa.getJobDiv() != EXPECTED) throw IllegalStateException`) — cmd 변조 fail-safe
5. p6spy로 PUT 1회당 SELECT 쿼리 수 회귀 검증 — 어댑터 fetch + consoled 조회만 발사 (jobDiv lookup 0건)

**사례**: `<commit-hash>` (2026-05-16) — Sea Master Update 흐름 PUT 9→4 쿼리 절감(2단계). 1단계: `SeaMasterPersistencePort.update` 시그니처 void→MasterBl + 첫 findEntityById → findJobDivById + 어댑터 attached domain 반환(9→5). 2단계: `findJobDivById` → `MasterBlJobDiv.valueOf(cmd.jobDiv())` cmd 신뢰(5→4). AIR Master는 기존 흐름 유지 (Air Master 마이그레이션 시 동일 패턴 적용 — §13.9 잔여 항목).
