...
| ㉕ | **Master Detail 응답 root 필드 `desc`(MasterBlDesc) — SEA/AIR 공통 SSOT, `<mode>Detail.desc` 별도 두기 금지** | Sea Master Entry 마이그레이션 마무리 라운드에서 발견: `MasterBlDetailResponse`/`MasterBlDetailResult`에 root `desc` 미노출 + `SeaDetailResponse.desc`(SeaDescView)에만 노출되어 FE Panel(root `desc.marks/description/descClause1/descClause2` register)에 화면 빈 값. 도메인 `MasterBlDesc`는 SEA/AIR 공통 자식이라 root 노출이 올바른 SSOT. `SeaDescProjection` → **`DescProjection`** rename, SEA/AIR 모두 재사용. (2026-05-16, 20293538) | 본 절 신설 | **Master B/L 도메인의 1:1 sub-entity(`<Mode>Desc` 같은 SEA/AIR 공통 자식)는 root에 노출, `<mode>Detail.desc` 별도 두기 금지**. plan 단계에서 (a) BE Detail Response/Result record root 필드 grep — `desc` 존재 확인, `<mode>Detail.desc`는 0건 (b) `DescProjection`/`DescView`(SEA/AIR 공통 record)인지 확인, mode별 별도 record 신설 금지 (c) FE root `desc` 필드 schema 정의 + Panel register/Controller name이 `desc.*`(root) 통일 (d) `<mode>Detail.desc` 흔적 grep 0건. **Air Master 진입 시 `airDetail.desc` 만들지 말 것** — root `desc`로 통일. |
| ㉖ | **Master Entry root 필드 4종 풀스택 정합 (mainItemName/hsCode/settlePartnerCode/desc)** | Sea Master Entry detail 응답에 도메인 보유 4 필드(`MasterBl.java:59~60` mainItemName/hsCode, :61 settlePartnerCode CustomerCode VO, :140 MasterBlDesc desc)가 누락되어 FE 화면(`master-cargo-doc-panel.tsx` Main Item/HS Code/Settle Partner, `master-panels.tsx` Marks/Description/Remark)에 빈 값. 원인: `MasterBlDetailResult`/`MasterBlDetailResponse`/`MasterBlFactory.toDetailResult` 3축 모두 미노출. FE 측 추가 결함: schema 중복 필드(`cargoMainItem`/`cargoHsCode`/`settlePartner`)가 BE 도메인 이름(`mainItemName`/`hsCode`/`settlePartnerCode`)과 별개로 존재해 sync 깨짐. (2026-05-16, 20293538 + 5f3b3f0d) | 본 절 신설 + §6.49 ⑬ 보강 | Master 도메인 추가/변경 시 **BE 도메인 필드 ⊂ DetailResponse 필드** + **FE schema 키 = BE 도메인 이름 SSOT** 정합 의무. plan 단계에서 (1) 도메인 필드 grep(`grep "private.*[A-Z].*;" <Domain>.java`) → DetailResult/Response/Factory 3축 노출 grep → 누락 시 BE 작업 단위 추가 (2) FE schema에 BE 도메인 이름과 다른 alias 필드(`cargoXxx`/`xxxAlias`) grep — 발견 시 제거하고 BE 도메인 이름 SSOT로 통일 (3) Panel register name이 BE 도메인 이름 매핑. **Air Master 진입 시 SEA에서 정합된 4 필드는 그대로 root 노출 — AIR airDetail nested 별도 둘 필요 없음**. |
| ㉗ | **Master Entry consolidated-hbl-list 별도 badge 영역 금지 — House B/L List 패널이 SSOT** | Sea Master `master-bl-entry.tsx`에 `detail.consolidatedHouseBls.map((hbl) => <span className="badge">{hbl.hblNo}</span>)` 별도 badge 영역이 잔존, House B/L List 패널(`master-house-bl-grid.tsx`)이 표 형태로 동일 정보 제공하므로 중복 fallback. CSS 클래스(`.consolidated-hbl-list`) 정의 부재로 스타일 영향 없으나 사용자 화면에 의미 없는 badge 한 줄 잔존. (2026-05-16, c80f40db) | 본 절 신설 | Master Entry에 콘솔된 자식 HBL 표시는 **House B/L List 패널(`master-house-bl-grid.tsx`) SSOT**. plan 단계에서 (a) `master-bl-entry.tsx`/`<domain>-entry.tsx`에 `consolidatedHouseBls` 별도 표시 영역(badge/list) grep — 발견 시 제거 (b) `ConsolidatedHouseBlSummary` import도 같이 제거. **Air Master 진입 시 동일 패턴 — 별도 badge 영역 만들지 말 것**. |
| ㉘ | **Master House B/L List hbl_no 셀 더블클릭 → House Entry 진입 패턴** | Sea Master Entry 내 House B/L List 패널(`master-house-bl-grid.tsx`)에서 hbl_no 셀 더블클릭 시 해당 HBL의 House Entry로 자동 이동 + 조회 동작 미적용. B/L List 더블클릭 패턴(`house-bl-list-grid.tsx`/`sea-house-grid.tsx`)을 그대로 차용 — `invalidateQueries(["house-bl","detail",id]) → clearDraft → setFocus(houseBl(variantKey), id) → sessionStorage hot-marker → addTab → router.push("/fms/house-bl/<variantKey>/entry")`. **master variant.key(sea-exp/sea-imp/air-exp/air-imp)를 그대로 house variant.key로 매핑** — 비즈니스 룰: SEA Master에 SEA HBL만, AIR Master에 AIR HBL만 콘솔. (2026-05-16, 5a8cbbb6) | 본 절 신설 + §6.16 보강 | Master Entry 내 House B/L List 그리드의 hbl_no 셀에 onDoubleClick + `e.stopPropagation()` (행 onClick 버블링 차단) + cursor pointer + title 의무. 더블클릭 핸들러는 `house-bl-list-grid.tsx:43~51` 패턴 + `sea-house-grid.tsx:48~58`의 hot-marker sessionStorage 결합. **master-house-bl-grid.tsx는 SEA/AIR 공통 컴포넌트라 Air Master 진입 시 본 패턴 별도 작업 0건 — 단, AIR-specific 그리드 신설 시 동일 핸들러 카피 의무**. |
...
24. **Master 도메인 1:1 sub-entity는 root 노출 SSOT** — `MasterBlDesc` 같은 SEA/AIR 공통 자식은 `MasterBlDetailResponse`/`MasterBlDetailResult` **root**에 `desc` 필드로 노출 의무. `SeaDetailResponse.desc`/`AirDetailResponse.desc` 같은 mode별 nested 노출 금지. plan 단계에서 (a) 도메인 `private MasterBlDesc desc` 같은 1:1 sub-entity grep (b) Response/Result root 필드에 동일 키 grep (c) `<mode>Detail.desc`/`<Mode>DescView` 같은 mode별 record 존재 시 root로 승격 + `DescProjection`/`DescView` 공통 record로 rename. 매트릭스 ㉕ 참조 — **Air Master 진입 시 root `desc` 그대로 사용, `airDetail.desc` 신설 금지**.
25. **Master 도메인 필드 ⊂ DetailResponse 정합 (mainItemName/hsCode/settlePartnerCode 등)** — Master 도메인 보유 필드 중 form 편집 가능 필드는 모두 BE Detail Response에 노출 의무. FE schema에 BE 도메인 이름과 다른 alias 필드(`cargoMainItem`/`cargoHsCode`/`settlePartner` 같은 중복 필드) 금지 — BE 도메인 이름 SSOT. plan 단계에서 `grep "private.*[A-Z].*;" <Domain>.java` ↔ `<Domain>DetailResponse`/`<Domain>DetailResult` 필드 set 비교. FE 측 `grep -i "cargo|alias"` schema 필드 발견 시 BE 도메인 이름으로 통일. 매트릭스 ㉖ 참조.
26. **Master Entry consolidated-hbl-list 별도 badge 영역 금지 + House B/L List 그리드 hbl_no 더블클릭 진입** — Master Entry에서 콘솔된 자식 HBL 표시는 House B/L List 패널(`master-house-bl-grid.tsx`) SSOT. `master-bl-entry.tsx`에 `consolidatedHouseBls.map((hbl) => <span className="badge">...)` 별도 표시 영역 신설 금지. House B/L List 그리드의 hbl_no 셀에는 onDoubleClick(invalidate → clearDraft → setFocus → hot-marker → addTab → router.push) + e.stopPropagation 의무 — master variant.key를 그대로 house variant.key로 매핑. 매트릭스 ㉗·㉘ 참조 — **Air Master 진입 시 master-house-bl-grid.tsx는 SEA/AIR 공통이라 본 항목 별도 작업 0건**.
27. **MasterBlFactory SRP 분리 — `<Mode>SubFactory` 신설 권장 패턴** — Master 도메인 Factory가 300줄 초과 시 (Sea Master 마이그레이션 시 383줄 → 265+118+60줄로 분리) `<Mode>SubFactory` 신설로 SRP. 공통 sub-entity(dim/scheduleLeg/airCharge/desc)는 `MasterBlSubFactory`, mode 확장 필드(SEA: linerCode/vesselVoyage/serviceTerm/blType/onboardDate/rton/lineBkgNo/issueDate)는 `MasterBl<Mode>SubFactory`. **Air Master 진입 시 `MasterBlAirSubFactory` 신설 권장** — AIR 확장 필드(linerCode/airline/flightNo/scheduleLegs/airCharges 등) 응집. 매트릭스 ㉕·㉖과 함께 적용.
...
3. Sea Master Entry — **완료 (2026-05-15, Phase 0~9, commits: 3df084a·8ca8ca3·adc2f3d·cb30351·3d6016a·a0d4882·747cf48·5ffcd3e)**
4. **Air Master Entry — 다음 타깃 (진입 전 §13.9 의무 점검 + §13.3 ChangeBlNoModal 공통화 검토 시점)**
...
### 13.8 Sea Master 진입 전 의무 점검 결과 (§13.7 재사용) — **Sea Master 적용 완료 (2026-05-15)**

> **Sea Master Entry Phase 0~9 완료 선언 (2026-05-15)**. §13.7 체크리스트 5항목 + Air House에서 적용 확인된 추가 항목 전수 재적용됨. Air Master 진입 시 본 §13.8과 §13.7 동일 체크리스트 재사용.

| § | 점검 항목 | Sea Master 적용 상태 | 적용 commit |
|---|---|---|---|
| §6.55 | Detail 응답 nested object null → `empty()` 정적 팩토리 | **완료** — `MasterBlDetailResponse.SeaDescView.empty()` + `MasterBlFactory.toSeaDescProjection` null 분기 | `8ca8ca3` (Phase 2) |
| §6.56 | useFieldArray race swipe — `resetVersion` key prop 버저닝 | **완료** — `MasterMainTab key={resetVersion}` + `FreightTab key={resetVersion}` 부착 + `useState resetVersion` | `747cf48` (Phase 7) |
| §6.57 | Delete/New 후 자식 탭 stale race 차단 | **완료** — `handleResetEntry` / `deleteMutation.onSuccess` 양쪽에 `setResetVersion(v=>v+1)` + `detailLoadedRef.current=false` + `removeQueries` + `clearDraft` | `747cf48` (Phase 7) |
| §6.58 | onSave 가드 `!isEdit` 신규 저장 차단 안티패턴 | **완료** — `master-bl-entry.tsx` `handleSave`/`mutationFn` `!isEdit` 가드 0건 (Delete 버튼만 정상 사용 2건) | Phase 7 검증 (수정 없음) |
| §6.59 | ComboBox label 매핑 정합 (BE EnumRegistryFactory + FE useEnumOptions) | **완료** — dev preview catalog Sea Master ENUMs 섹션(LoadType/ServiceTerm/BlType/PackType) 신설 | `a0d4882` (Phase 6) |

**Sea Master 마이그레이션 중 적용 확인된 추가 항목**:

- §6.37 sub-set 매퍼 master 참조 NULL 덮어쓰기 — **완료** `cb30351` (Phase 4) — `SeaMasterUpdatePersistenceAdapter` + `MasterBlSeaSubMapper` 3분기 분류 + `assignParties` null 가드
- §6.53 Create EXP/IMP 분기 검증 — **완료** `3df084a` (Phase 0) + `adc2f3d` (Phase 3) — `SeaMasterGroup`/`SeaImpMasterGroup` 신설 + Controller SEA 분기 동적 검증
- §6.54 POST 응답 ID-only 컨벤션 — **완료** `adc2f3d` (Phase 3) — `Map.of("id", id)` 응답 + EXACT endpoint `find-by-mbl-no`
- §6.52 `@JsonInclude(NON_NULL)` + FE zod void 응답 파싱 — **완료** `3d6016a` (Phase 5) — `master-bl.ts` update void 응답 `(json ?? {}).data == null` 패턴 적용
- §6.49 ⑨ `useBlDraftSync` `didRestoreFromDraftRef` 가드 — **완료** `5ffcd3e` (Phase 8) — form.reset useEffect에 가드 추가
- §6.49 ⑩ mutation onSuccess SSOT 5요소 — **완료** `5ffcd3e` (Phase 8) — invalidate detail / sessionStorage hot / setFocus / clearDraft / detailLoadedRef=false + List 자동 invalidate 0건 + router.push 0건
- §6.49 ⑯ TOOLBAR_TO_FIELD 4축 정합 root vs nested — **완료** `5ffcd3e` (Phase 8) — Master Ref/MBL No/Shipment Type=root, Line Bkg/Load/Service/BL Type=`seaDetail.*` nested

**주요 product 결정 (Sea Master 단독)**:
- `incoterms`/`deliveryCode` Master 도메인 미적용 (Sea House `HouseBl.deliveryCode`와 PRD 차이)
- `remark` SEA/AIR 확장 분산 유지 (House 격리 정합)
- Toolbar `Status` 필드 FE 제거 (BE 미지원)
- AIR Master 코드 격리 원칙 (AIR 분기 0건 수정)

**Sea Master 누적 후속 항목 (별도 처리 필요)**:
- `master-bl-entry.tsx` 332줄 (300 초과, 500 미달) — handlers/page-head 추출 분리 검토
- AIR Master 분기 schema 미정의 필드(`seaDetail.linerName`/`polName`/`signature`/`issuePlace`) — Air Master Phase에서 처리
- `non-bl/__tests__/map-non-bl-detail.test.ts` `jobDiv` 오류 — commit `f429975`(2026-05-11) 기존 회귀, 테스트 수정 사용자 승인 필요
- `master-cargo-doc-panel.tsx` warning 2건 (`Controller` import / `form` 파라미터 미사용)

**§6.61 신설 — Sea Master Phase 3 회복 라운드 학습** (WebMvcTest `@MockitoBean Validator` 추가 금지 + `@Validated` 클래스 레벨 유지 + happyPath request body 그룹별 `@NotBlank` 필수 보강): phase3-data-flow.md §6.61 신설.

### 13.9 Air Master 진입 전 의무 점검 (§13.7·§13.8 재사용 + 본 세션 신설 SSOT)

> **차기 타깃**: Air Master Entry (§13.6 ④). Sea Master 9 Phase + post-migration 풀스택 정합(2026-05-15 22:00 ~ 2026-05-16) 학습을 바탕으로 진입 전 의무 점검.

§13.7(§6.55~§6.59) + §13.8(Sea Master 적용 확인) 체크리스트 5항목 + Sea Master 단독 적용 추가 항목 7건은 **Air Master 진입 시 그대로 재사용**.

**Sea Master post-migration에서 신설된 매트릭스(2026-05-16, §6.49 ㉕~㉘) — Air Master 진입 전 의무 grep 5건**:

| § | 점검 항목 | Air Master 진입 점검 항목 | 적용 commit (Sea Master) |
|---|---|---|---|
| ㉕ | Master 1:1 sub-entity `desc` root 노출 SSOT | `MasterBlDetailResult`/`MasterBlDetailResponse` root에 `desc` 필드 존재 확인 + `airDetail.desc` 신설 금지 | `20293538` |
| ㉖ | Master 도메인 필드 ⊂ DetailResponse 정합 | `mainItemName`/`hsCode`/`settlePartnerCode` 4 필드 노출 확인 + FE schema `cargoXxx`/`settlePartner` alias 0건 | `20293538` + `5f3b3f0d` |
| ㉗ | consolidated-hbl-list badge 영역 금지 | `master-bl-entry.tsx`에 별도 badge 영역 신설 금지 (House B/L List 패널 SSOT) | `c80f40db` |
| ㉘ | House B/L List hbl_no 셀 더블클릭 진입 | `master-house-bl-grid.tsx`는 SEA/AIR 공통 — 별도 작업 0건. AIR-specific 그리드 신설 시 핸들러 카피 | `5a8cbbb6` |
| (SRP 27) | `MasterBlAirSubFactory` 신설 권장 | `MasterBlFactory` AIR 분기 작업이 30줄 초과 시 `MasterBlAirSubFactory`로 추출 (Sea Master 선례: `MasterBlSeaSubFactory` 118줄) | `81f8ffdc` |

**Air Master 도메인 특이 사항**:

- **컨테이너 미사용** — SEA Master의 `ConsoledSeaContainerView`/`master-container-grid.tsx` 패턴(2026-05-15 `f32e70b9`)은 **AIR에 미적용**. AIR Master는 dim 그리드(`MasterBlDim`) 별도 패턴 필요 — `house_bl` 대비 `master_bl`의 dim FK 위치 확인(`master_bl_air.master_bl_air_id` Phase 4 ER 재구조화 적용분).
- **AIR 확장 필드 격리 원칙 (Sea Master에서 적용 확인됨)** — `MasterBlAir*` / `airDetail.*` / `AirMasterGroup`/`AirImpMasterGroup` 분기는 SEA 도메인 코드에서 0건 수정. Sea Master 마이그레이션 product 결정 그대로 유지.
- **누락 schema 필드** — Sea Master 누적 후속(history.md `5ffcd3e` 항목)에서 식별: `seaDetail.linerName`/`polName`/`signature`/`issuePlace` 4 필드는 AIR Master 사용 가능성 검토 — `airDetail.signature`/`issuePlace`는 AIR EXP 도메인 필수 후보(§13.7 Air House `air-issue-panel.tsx` 패턴 참고).
- **§12.1 VolumeDivisor 공통화 결정** — 이미 Non B/L + Truck B/L 2 도메인 보유. Air Master 마이그레이션 시 `HouseBl` 부모 승격 검토 시점 — plan 단계 결정 후 진행.

**Air Master Plan 단계 의무**:

1. §6.48 7항목 + §6.49 ㉕~㉘ 매트릭스 + §13.7~§13.9 체크리스트 grep 후 누락 0건 보고.
2. `master-bl-entry.tsx` 332줄(Sea Master 누적 후속) — Air Master 진입 시 추가 라인 가산 전에 분리 작업 단위 우선 검토 (handlers / page-head / form.reset mapper 추출).
3. `MasterBlAirSubFactory` 신설 + `master_bl_air` JPA dim/scheduleLeg/airCharge 분리 영속 패턴(Phase 4 ER 재구조화 적용분) 정합.
4. AIR-specific 자식 그리드(scheduleLegs/airCharges/dims) BE detail 응답 노출 — Sea Master Phase 2 (`8ca8ca3`) 패턴 차용해 `AirDetailResponse` 신설.
5. WebMvcTest 동적 Validator 회피 (§6.61) — `AirMasterGroup`/`AirImpMasterGroup` 검증 시 `@MockitoBean Validator` 추가 금지.
