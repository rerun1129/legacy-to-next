...
2. **Air House Entry — 다음 타깃 (2026-05-14 SSOT 5건 적용 의무)**
...
### 13.7 Air House 진입 전 의무 점검 (2026-05-14 SSOT 5건 적용)

Sea House Entry 마이그레이션 마무리 + Truck B/L desc 정합 과정에서 phase3-data-flow.md §6.55~§6.59 SSOT 5건 신설. **Air House Entry plan 단계에 의무 grep/적용 체크리스트로 포함**:

| § | 점검 항목 | Air House 적용 방법 | 검출 방법 |
|---|---|---|---|
| §6.55 | Detail 응답 nested object null → `empty()` 정적 팩토리 | `AirHouseDetailResponse.from` + `HouseBlFactory.toAirDescView`(혹은 동등 메서드) 양 경로에 `DescView.empty()` 분기 + projection record에 `empty()` 추가 | BE: `AirHouse*Response` `result.desc()` null 분기 grep / FE: `Invalid air-house detail response` 에러 발생 시 §6.55 적용 |
| §6.56 | useFieldArray race swipe — `resetVersion` key prop 버저닝 | Air House Cargo/Charge 자식 패널에 `resetVersion` prop chain + 자식 root에 `key={resetVersion}` | `useFieldArray` 사용 자식 컴포넌트 grep |
| §6.57 | Delete/New 후 자식 탭 stale race 차단 | Delete/New 핸들러에 자식 grid local state 초기화 + `form.reset` 동시 호출 | 자식 탭이 `useQuery` 별도 호출인지 detail 응답 내부인지 사전 확인 |
| §6.58 | onSave 가드 `!isEdit` 신규 저장 차단 안티패턴 | onSave 시작부에 `!isEdit`/`mode !== "edit"` 가드 grep, edit 전용 후처리에만 가드 적용 | `air-house-entry.tsx` `onSave`/`handleSave` 함수 시작부 grep |
| §6.59 | ComboBox label 매핑 정합 (BE EnumRegistryFactory + FE useEnumOptions) | Air House 도메인의 enum 필드(BlType/ServiceTerm/Incoterms/ContainerType/MovementMode 등) 모두 dev preview catalog 시각 확인 | `lint`/`build` PASS만으로 검출 불가 → 강력 새로고침 시각 검증 의무 |

**Sea House 잔여 적용 잠재 항목** (Air House 마이그레이션 중 마주칠 가능성)

- §6.37 sub-set 매퍼 master 참조 NULL 덮어쓰기 (a04f066 Sea House에서 발견) — Air House sub-set 매퍼에도 동일 점검
- §6.45 `containerType` detail 응답 매핑 — Air House 도메인에서 컨테이너 사용 시 동일 정합 (27790e0·e830f97 패턴)
- §6.51 ServiceTerm 매칭 `fromLabel` vs `valueOf` 비대칭 (7f91282 Sea/Master 통일) — Air에서도 동일 SSOT 적용
- §6.52 `@JsonInclude(NON_NULL)` + FE zod void 응답 파싱 (dd76d64 SEA) — Air void 응답에도 동일
- §6.53 Create EXP/IMP 분기 검증 — `AirGroup`/`AirImpGroup` 인터페이스 신설 + `@ValidationGroups` 매핑 (Sea의 `SeaGroup`/`SeaImpGroup` 모범)
- §6.54 POST 응답 ID-only 컨벤션 — `createHouseBl` SEA 정렬과 동일 흐름이 AIR JobDiv 분기로도 보장되는지 확인
