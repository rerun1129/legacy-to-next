...
- **Round 3 후속 결함 (open)**:
  - ~~SEA `containers` + `desc`(marks/description/descClause1/descClause2) BE detail 응답 노출 누락~~ — **해소 (2026-05-13)**. `SeaDetailProjection`/`SeaDetailResponse`에 `containers`/`desc` 필드 + nested record 추가. FE `HouseBlDetail` 타입·zod 스키마·`form.reset` 매핑 동기. `licenses`는 미사용 도메인으로 FE/docs에서 전수 제거(BE는 이미 0건).
  ...
  - `house-bl-entry.tsx` 500줄 (300 초과, 500 경계 — 강제 분리 미해당) → 권장 분리 후보: page-head buttons / form.reset 매핑(약 100줄) / mutation 핸들러.
